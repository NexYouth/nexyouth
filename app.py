...
def serve_static(filename):
    response = send_from_directory(os.path.join(os.path.dirname(__file__), 'static'), filename)
    if filename.endswith(('.mp4', '.webm', '.mov')):
        response.cache_control.public = True
        response.cache_control.max_age = 31536000
        response.cache_control.immutable = True
    elif filename.endswith(('.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.ico')):
        response.cache_control.public = True
        response.cache_control.max_age = 2592000
        response.cache_control.immutable = True
    elif filename.endswith(('.css', '.js', '.woff', '.woff2')):
        response.cache_control.public = True
        response.cache_control.max_age = 2592000
    return response
...
def robots():
    return send_from_directory(os.path.join(os.path.dirname(__file__), 'public'), 'robots.txt')
